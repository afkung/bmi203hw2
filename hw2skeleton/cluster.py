...
def kmeansCluster(points, centroids, k):
	...
	Input: ActiveSite instances to cluster, centroids of cluster's vectors, number of clusters
	...
	for item in points:
		...
		for index in range(k):
			distance = ((item[0]-centroids[index][0])**2 + (item[1]-centroids[index][1])**2 + (item[2]-centroids[index][2])**2)**0.5
...
def cluster_by_partitioning(active_sites,number_clusters):
	...
	centroid_indices = np.random.choice(len(vectors),k,replace = False)
	for index in range(k):
		centroids[index] = vectors[centroid_indices[index]]
	# run clustering for n iterations
	iterations = 100
	for iteration in range(iterations):
		# cluster points based on centroids
		clusters = kmeansCluster(vectors, centroids,k)
		# find new centroids
		for index in range(k):
